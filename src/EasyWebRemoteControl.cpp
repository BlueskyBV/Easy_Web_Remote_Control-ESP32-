...
void EasyWebRemoteControl::begin(const char* ssid, const char* password) {
...
// Callbacks
void EasyWebRemoteControl::onForward(void (*func)()) { forwardCallback = func; }
void EasyWebRemoteControl::onBackward(void (*func)()) { backwardCallback = func; }
...
// Slider
int EasyWebRemoteControl::getSpeed() { return currentSpeed; }
void EasyWebRemoteControl::setInitialSpeed(int val) { if(val<0) val=0; if(val>255) val=255; currentSpeed=val; }
...
String EasyWebRemoteControl::buildHtmlPage(){
    // Serialize configuration maps
    String timersJS="const actionTimers={"; for(auto &kv:actionTimers){timersJS+="\""+kv.first+"\": "+String(kv.second)+",";} if(!actionTimers.empty()) timersJS.remove(timersJS.length()-1); timersJS+="};";
    String tapsJS="const tapConfig={"; for(auto &kv:tapSettings){tapsJS+="\""+kv.first+"\": "+String(kv.second)+",";} if(!tapSettings.empty()) tapsJS.remove(tapsJS.length()-1); tapsJS+="};";
    String holdJS="const holdConfig={"; for(auto &kv:holdSettings){holdJS+="\""+kv.first+"\": "+String(kv.second)+",";} if(!holdSettings.empty()) holdJS.remove(holdJS.length()-1); holdJS+="};";
    String delayJS="const delayConfig={"; for(auto &kv:delaySettings){delayJS+="\""+kv.first+"\": "+String(kv.second)+",";} if(!delaySettings.empty()) delayJS.remove(delayJS.length()-1); delayJS+="};";

    // Slider HTML
    String sliderUI="";
    if(sliderEnabled){
        sliderUI = "<div class=\"row\"><input type=\"range\" id=\"speedSlider\" min=\"0\" max=\"255\" value=\"" + String(currentSpeed) +
                   "\"><span id=\"speedValue\">" + String(currentSpeed) + "</span></div>";
    }

    // Full page with JS logic including pressed state
    String page = "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"><title>Remote Control</title>"
                  ...
                  "@media(max-width:480px){button{width:80px;height:80px;font-size:28px;}input[type=range]{width:180px;}}</style></head><body>"
...
let ws;
function connectWS(){ws=new WebSocket(gateway); ws.onopen=()=>console.log('ws open'); ws.onclose=()=>{console.log('ws closed'); setTimeout(connectWS,1000);}; ws.onerror=(e)=>console.log('ws err',e);}
connectWS();
...
const pressActive={}; const pressStart={}; const holdTimer={}; const actionActive={}; const startDelayTimer={}; const tapCount={}; const tapResetTimer={}; const pendingStopTimer={};
...
function startActionForButton(btnId, triggeredBy){
    ...
    function doStart(){
        ...
        if(triggeredBy==="release" && actionTimer>0){
            clearPendingStop(btnId);
            pendingStopTimer[btnId]=setTimeout(()=>{sendCommand("stop"); pendingStopTimer[btnId]=null; actionActive[btnId]=false; if(btn) btn.classList.remove("pressed");}, actionTimer);
        }
    }
...
buttons.forEach(b=>{
    const id=b.id;
    pressActive[id]=false; pressStart[id]=0; holdTimer[id]=null; actionActive[id]=false; startDelayTimer[id]=null; tapCount[id]=0; tapResetTimer[id]=null; pendingStopTimer[id]=null;
    ...
    function onUp(e){ e&&e.preventDefault&&e.preventDefault(); const now=Date.now(); const elapsed=now-(pressStart[id]||now); pressActive[id]=false; clearHoldTimeout(id);
        ...
        if(tapsNeeded>0){
            if(holdMs===0 || elapsed>=holdMs){ tapCount[id]=(tapCount[id]||0)+1; clearTapReset(id); tapResetTimer[id]=setTimeout(()=>{tapCount[id]=0; tapResetTimer[id]=null;}, TAP_RESET_MS);
...
function stopAll(){ Object.keys(startDelayTimer).forEach(k=>{if(startDelayTimer[k]){clearTimeout(startDelayTimer[k]); startDelayTimer[k]=null;}});
    ...
    Object.keys(pressActive).forEach(k=>{pressActive[k]=false; actionActive[k]=false; tapCount[k]=0; const btn=document.getElementById(k); if(btn) btn.classList.remove("pressed");});
